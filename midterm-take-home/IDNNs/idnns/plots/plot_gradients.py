...
def plot_gradients(name_s=None, data_array=None, figures_dir=''):
    ...
    if data_array == None:
        ...
        data_array = plt_ut.get_data(name_s)
    # print(data_array)
    ...
    if conv_net:
        ...
        print("NUMBER OF batches = ", num_of_batchs)
        num_of_layers = int(len(gradients[0][0]) / 2)
    ...
    epochsInds = (data_array['params']['epochsInds']).astype(np.int_)
    ...
    all_gradients = np.empty(num_of_layers, dtype=np.object_)
...
def calc_mean_var_loss(epochsInds,loss_train):
    ...
    print('NUM EPOCHS = ', num_of_epochs)
    print('SHAPE OF LOSS_TRAIN = ', loss_train.shape)
    print('NUMBER OF DIMS OF LOSS TRAIN = ', loss_train.ndim)
    #Average over the batchs
    loss_train_mean = np.mean(loss_train)
    #The diff divided by the sampled indexes
    d_mean_loss_to_dt = np.sqrt(np.abs(loss_train / epochsInds[:]))
    var_loss = []
    #Go over the epochs
    for epoch_index in range(num_of_epochs):
...
def plot_loss_figures(data_array, fig_size=(14, 10), xlim = None, y_lim = None):
    epochsInds = (data_array['params']['epochsInds']).astype(np.int_) # changed np.int to np.int_
    print("SHAPE OF LOSS_TRAIN = ", np.array(data_array['loss_train']).shape)
    dif_var_loss, diff_mean_loss = calc_mean_var_loss(epochsInds, np.squeeze(np.array(data_array['loss_train']))) # removed np.squeeze()
